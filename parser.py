...
def parse_html(html: str) -> Dict:
    ...
    try:
        administration = soup.find("a", class_="card-badge text-wrap text-left badge badge-gray-200 p-2 m-1 font-weight-light course-data course-data__online").text
    except:
        administration = ""
...
if __name__ == "__main__":
    ...
    # Create folder courses inside the project
    os.makedirs(f"courses", exist_ok=True)
    
    # Extract all file paths from every HTML saved
    htmls = get_htmls("pages")
    ...
    for i, html in enumerate(htmls):
        ...
        if os.path.getsize(html) / 1024 > 10:
            ...
            df_courses.to_csv(f"courses/course_{i+1}.tsv", sep="\t", index=False)
