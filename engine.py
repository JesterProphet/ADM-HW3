...
def retrieve_courses(query, vocabulary, k=None):
    ...
    tfidf_query = dict(set(zip(feature_names, tfidf_scores)))
    ...
    # Collect all course descriptions from matched courses
    course_descriptions = {}
    for course_id in course_ids:
        course_descriptions[course_id] = []
        for word, tfidf_score in vocabulary.items():
            for vocabulary_course_id, vocabulary_tfidf_score in tfidf_score:
                if vocabulary_course_id == course_id:
                    course_descriptions[course_id].append((word, vocabulary_tfidf_score))
    ...
    for course_id in course_ids:
        ...
        # Retrieve all words in course description and corresponding tfidf scores
        course_description = dict(course_descriptions[course_id])

        # Extract the common words from course description and query string
        all_words = set(course_description.keys()).union(tfidf_query.keys())

        # Transfrom into vectors
        course_vector = np.array([course_description.get(word, 0) for word in all_words])
        query_vector = np.array([tfidf_query.get(word, 0) for word in all_words])
        
        # Calculate cosine similarity between current course and query
        similarity = cosine_similarity(course_vector.reshape(1, -1), query_vector.reshape(1, -1))
