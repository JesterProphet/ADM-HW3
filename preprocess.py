...
def preprocess_text(text: str) -> str:
    """
    Function that takes a text as a string, removes stopwords and punctuation, checks if it only contains alphabetical letters,
...
def get_fee(fee_text: str) -> float:
    """
    Function that takes a string and extracts the (maximum) fee in EUR.
    ...
    for root, dirs, files in os.walk("courses"):
        for file in files:
            ...
            if not df.empty: 
                ...
                # Drop column fees (€) if already exists
                try:
                    df = df.drop("fees (€)", axis=1)
                except:
                    pass
                
                # Drop column preprocessed description if already exists
                try:
                ...
                if pd.notna(fees):
                    ...
                    if all(string not in fees.lower() for string in no_fees_keywords) and not is_valid_link(fees):
                        ...
                        # Check if a fee was retrieved and exclude outliers
                        if fee:
                            if float(fee) <= 150000:
                                df["fees (€)"] = fee
